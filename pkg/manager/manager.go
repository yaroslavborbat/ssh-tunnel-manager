...
func NewSSHTunnelManager(tunnels []config.Tunnel, startSSHAgent bool) SSHTunnelManager {
...
func (m *sshTunnelManager) Run(ctx context.Context) error {
	newCtx, cancel := context.WithCancel(ctx)
	defer cancel()

	if m.startSSHAgent {
		slog.Info("Start ssh-agent")
		if err := m.startAgent(newCtx); err != nil {
			return err
		}
	}
...
func (m *sshTunnelManager) startAgent(ctx context.Context) error {
...
func (m *sshTunnelManager) sshAdd(passPhrasePath, privateKeyPath string) error {
...
func (m *sshTunnelManager) runTunnel(ctx context.Context, tunnel *config.Tunnel) error {
...
func (m *sshTunnelManager) makeSShTunelCommandArgs(tunnel *config.Tunnel) (string, []string) {
	name := sshCMD
	args := []string{
		"-o",
